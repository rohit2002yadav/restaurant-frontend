...
import { HiMail, HiLockClosed, HiUser, HiPhone, HiOfficeBuilding, HiEye, HiEyeOff } from 'react-icons/hi';
...
export default function Register() {
  ...
  const [form, setForm] = useState({ restaurant_name: '', name: '', phone: '', email: '', password: '', confirm: '' });

  const set = (key) => (e) => setForm(f => ({ ...f, [key]: e.target.value }));

  const validate = () => {
    const e = {};
    if (role === 'admin' && !form.restaurant_name) e.restaurant_name = 'Restaurant name is required';
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const payload = role === 'admin'
        ? { restaurant_name: form.restaurant_name, name: form.name, phone: form.phone, email: form.email, password: form.password }
        ...
        <div className="anim-scale-in" style={{ width: '100%', maxWidth: 420 }}>
          ...
          <div className="role-toggle mb-6">
            {['customer', 'admin'].map(r => (
              <button key={r} type="button" className={`role-toggle-btn${role === r ? ' active' : ''}`} onClick={() => setRole(r)}>
          ...
          <div className="card">
            <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              {role === 'admin' && (
                <Input label="Restaurant Name" type="text" placeholder="Spice Garden"
                  icon={HiOfficeBuilding} value={form.restaurant_name}
                  onChange={set('restaurant_name')} error={errors.restaurant_name} />
              )}
              ...
              <Input label="Confirm Password" type="password" placeholder="Repeat password"
                icon={HiLockClosed} value={form.confirm} onChange={set('confirm')} error={errors.confirm} />
